...
## ---- plot_markov_chain_burn_in
n <- 20
p <- 0.3
chain_length <- 5000
X <- binomial_markov_chain(n, p, chain_length)

qplot(c(0:199), X[1:200], geom="step", xlab = expression('X'[n]), ylab = "State")
...
ggplot() +
  geom_bar(data = data.frame(X), aes(x = X, y = (..count..)/sum(..count..)),
           fill = "grey") +
  geom_point(data = data.frame(states), 
             aes(x = states, y = dbinom(states, n, prob=p))) +
