...
binomial_markov_chain <- function(n, p, chain_length) {
  ...
  for (i in 1:n-2) {
    alpha[i+1, i] <- min(i/(n-i+1) * (1-p)/p, 1)
    alpha[i+1, i+2] <- min((n-i)/(i+1) * p/(1-p), 1)
  }
  ...
  for (i in 1:chain_length) {
    ...
    # If proposed state is accepted, set new state and update accepted count
    accept <- rbinom(1, 1, alpha[X_previous+1, X_proposed+1])
    if (accept == 1) {
      accepted <- accepted + 1
      X[i+1] <- X_previous <- X_proposed
    } 
    
    # Otherwise set new state equal to the previous state
    else {
      X[i+1] <- X_previous
    }
  }
